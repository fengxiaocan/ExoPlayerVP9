    ...
    if (headers != null) {
      ...
      try {
        ...
        if (bitrate > 0) {
          ...
          ExoLog.w(TAG, "Invalid bitrate: " + bitrateHeader);
          bitrate = Format.NO_VALUE;
        }
      } catch (NumberFormatException e) {
        ExoLog.w(TAG, "Invalid bitrate header: " + bitrateHeader);
      }
    }
    ...
    if (headers != null) {
      ...
      try {
        ...
        if (metadataInterval > 0) {
          ...
          ExoLog.w(TAG, "Invalid metadata interval: " + metadataIntervalHeader);
