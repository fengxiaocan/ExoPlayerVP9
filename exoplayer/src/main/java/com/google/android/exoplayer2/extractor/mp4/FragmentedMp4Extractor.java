  ...
  private void onEmsgLeafAtomRead(ParsableByteArray atom) {
    ...
    switch (version) {
      ...
      case 1:
        ...
        break;
      default:
        ExoLog.w(TAG, "Skipping unsupported emsg version: " + version);
        return;
    }
  ...
  private boolean readSample(ExtractorInput input) throws IOException {
    ...
    if (trackBundle == null) {
      ...
      if (bytesToSkip < 0) {
        ...
        ExoLog.w(TAG, "Ignoring negative offset to sample data.");
        bytesToSkip = 0;
      }
      input.skipFully(bytesToSkip);
  ...
  private static DrmInitData getDrmInitDataFromAtoms(List<LeafAtom> leafChildren) {
    ...
    for (int i = 0; i < leafChildrenSize; i++) {
      ...
      if (child.type == Atom.TYPE_pssh) {
        ...
        if (uuid == null) {
          ExoLog.w(TAG, "Skipped pssh atom (failed to extract uuid)");
        } else {
