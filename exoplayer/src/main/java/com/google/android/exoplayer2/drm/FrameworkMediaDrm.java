...
public final class FrameworkMediaDrm implements ExoMediaDrm {
  ...
  public static final Provider DEFAULT_PROVIDER =
      uuid -> {
        try {
          ...
          ExoLog.e(TAG, "Failed to instantiate a FrameworkMediaDrm for uuid: " + uuid + ".");
  ...
  public void setPlayerIdForSession(byte[] sessionId, PlayerId playerId) {
    if (Util.SDK_INT >= 31) {
      try {
        ...
        ExoLog.w(TAG, "setLogSessionId failed.");
      }
    }
  }
  ...
  private static byte[] addLaUrlAttributeIfMissing(byte[] data) {
    ...
    if (objectRecordCount != 1 || recordType != 1) {
      ExoLog.i(TAG, "Unexpected record count or type. Skipping LA_URL workaround.");
      return data;
    }
    ...
    if (endOfDataTagIndex == -1) {
      ExoLog.w(TAG, "Could not find the </DATA> tag. Skipping LA_URL workaround.");
    }
