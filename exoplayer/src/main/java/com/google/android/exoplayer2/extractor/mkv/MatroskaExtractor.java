...
public class MatroskaExtractor implements Extractor {
    ...
    if (track.trueHdSampleRechunker != null) {
      ...
      if (CODEC_ID_SUBRIP.equals(track.codecId)
        ...
        if (blockSampleCount > 1) {
          ExoLog.w(TAG, "Skipping subtitle sample in laced block.");
        } else if (blockDurationUs == C.TIME_UNSET) {
          ExoLog.w(TAG, "Skipping subtitle sample with no duration.");
        } else {
  ...
  private SeekMap buildSeekMap(
    ...
    if (lastDurationUs <= 0) {
      ExoLog.w(TAG, "Discarding last cue point with unexpected duration: " + lastDurationUs);
  ...
  protected static final class Track {
      ...
      switch (codecId) {
        ...
        case CODEC_ID_ACM:
          ...
          if (parseMsAcmCodecPrivate(new ParsableByteArray(getCodecPrivate(codecId)))) {
            ...
            mimeType = MimeTypes.AUDIO_UNKNOWN;
            ExoLog.w(TAG, "Non-PCM MS/ACM is unsupported. Setting mimeType to " + mimeType);
          }
          break;
        case CODEC_ID_PCM_INT_LIT:
          mimeType = MimeTypes.AUDIO_RAW;
    ...
    private static Pair<String, @NullableType List<byte[]>> parseFourCcPrivate(
      ...
      ExoLog.w(TAG, "Unknown FourCC. Setting mimeType to " + MimeTypes.VIDEO_UNKNOWN);
