...
import com.google.android.exoplayer2.util.ExoLog;
...
public final class MediaCodecUtil {
  ...
  public static void warmDecoderInfoCache(String mimeType, boolean secure, boolean tunneling) {
    try {
      ...
      ExoLog.e(TAG, "Codec warming failed", e);
    }
  }
      ...
      for (int i = 0; i < numberOfCodecs; i++) {
        ...
        try {
          ...
          if (Util.SDK_INT <= 23 && !decoderInfos.isEmpty()) {
            ...
            ExoLog.e(TAG, "Skipping codec " + name + " (failed to query capabilities)");
          } else {
            // Rethrow error querying primary codec capabilities, or secondary codec
            // capabilities if API level is greater than 23.
            ExoLog.e(TAG, "Failed to query codec " + name + " (" + codecMimeType + ")");
            throw e;
          }
        }
      }
  ...
  private static Pair<Integer, Integer> getDolbyVisionProfileAndLevel(
    ...
    if (parts.length < 3) {
      ...
      ExoLog.w(TAG, "Ignoring malformed Dolby Vision codec string: " + codec);
      return null;
    }
    ...
    if (profile == null) {
      ExoLog.w(TAG, "Unknown Dolby Vision profile string: " + profileString);
      return null;
    }
    String levelString = parts[2];
    @Nullable Integer level = dolbyVisionStringToLevel(levelString);
    if (level == null) {
      ExoLog.w(TAG, "Unknown Dolby Vision level string: " + levelString);
      return null;
    }
  ...
  private static Pair<Integer, Integer> getHevcProfileAndLevel(
    ...
    if (parts.length < 4) {
      ...
      ExoLog.w(TAG, "Ignoring malformed HEVC codec string: " + codec);
      return null;
    }
    // The profile_space gets ignored.
    Matcher matcher = PROFILE_PATTERN.matcher(parts[1]);
    ...
    if ("1".equals(profileString)) {
      ...
      ExoLog.w(TAG, "Unknown HEVC profile string: " + profileString);
      return null;
    }
    @Nullable String levelString = parts[3];
    @Nullable Integer level = hevcCodecStringToProfileLevel(levelString);
    if (level == null) {
      ExoLog.w(TAG, "Unknown HEVC level string: " + levelString);
      return null;
    }
    return new Pair<>(profile, level);
  }
  ...
  private static Pair<Integer, Integer> getAvcProfileAndLevel(String codec, String[] parts) {
    if (parts.length < 2) {
      ...
      ExoLog.w(TAG, "Ignoring malformed AVC codec string: " + codec);
      return null;
    }
    int profileInteger;
    int levelInteger;
    ...
    if (profile == -1) {
      ExoLog.w(TAG, "Unknown AVC profile: " + profileInteger);
      return null;
    }
    int level = avcLevelNumberToConst(levelInteger);
    if (level == -1) {
      ExoLog.w(TAG, "Unknown AVC level: " + levelInteger);
      return null;
    }
    return new Pair<>(profile, level);
  }
  ...
  private static Pair<Integer, Integer> getVp9ProfileAndLevel(String codec, String[] parts) {
    if (parts.length < 3) {
      ExoLog.w(TAG, "Ignoring malformed VP9 codec string: " + codec);
      return null;
    }
    int profileInteger;
    int levelInteger;
    ...
    if (profile == -1) {
      ExoLog.w(TAG, "Unknown VP9 profile: " + profileInteger);
      return null;
    }
    int level = vp9LevelNumberToConst(levelInteger);
    if (level == -1) {
      ExoLog.w(TAG, "Unknown VP9 level: " + levelInteger);
      return null;
    }
    return new Pair<>(profile, level);
  }
  ...
  private static Pair<Integer, Integer> getAv1ProfileAndLevel(
      String codec, String[] parts, @Nullable ColorInfo colorInfo) {
    if (parts.length < 4) {
      ExoLog.w(TAG, "Ignoring malformed AV1 codec string: " + codec);
      return null;
    }
    int profileInteger;
    int levelInteger;
    ...
    if (profileInteger != 0) {
      ExoLog.w(TAG, "Unknown AV1 profile: " + profileInteger);
      return null;
    }
    if (bitDepthInteger != 8 && bitDepthInteger != 10) {
      ExoLog.w(TAG, "Unknown AV1 bit depth: " + bitDepthInteger);
      return null;
    }
    int profile;
    ...
    if (level == -1) {
      ExoLog.w(TAG, "Unknown AV1 level: " + levelInteger);
      return null;
    }
    return new Pair<>(profile, level);
  }
  ...
  private static Pair<Integer, Integer> getAacCodecProfileAndLevel(String codec, String[] parts) {
    if (parts.length != 3) {
      ExoLog.w(TAG, "Ignoring malformed MP4A codec string: " + codec);
      return null;
    }
    try {
