...
/* package */ final class StreamVolumeManager {
  ...
  public StreamVolumeManager(Context context, Handler eventHandler, Listener listener) {
    ...
    try {
      ...
      ExoLog.w(TAG, "Error registering stream volume receiver", e);
    }
  }
  ...
  public void release() {
    if (receiver != null) {
      try {
        ...
        ExoLog.w(TAG, "Error unregistering stream volume receiver", e);
      }
      receiver = null;
    }
  }
  ...
  private static int getVolumeFromManager(AudioManager audioManager, @C.StreamType int streamType) {
    ...
    try {
      ...
      ExoLog.w(TAG, "Could not retrieve stream volume for stream type " + streamType, e);
      return audioManager.getStreamMaxVolume(streamType);
    }
  }
