...
public final class VorbisUtil {
  ...
  public static Metadata parseVorbisComments(List<String> vorbisComments) {
    ...
    for (int i = 0; i < vorbisComments.size(); i++) {
      ...
      if (keyAndValue.length != 2) {
        ExoLog.w(TAG, "Failed to parse Vorbis comment: " + vorbisComment);
        continue;
      }
      ...
      if (keyAndValue[0].equals("METADATA_BLOCK_PICTURE")) {
        ...
        try {
          ...
          ExoLog.w(TAG, "Failed to parse vorbis picture", e);
        }
      } else {
  ...
  private static void readMappings(int channels, VorbisBitArray bitArray) throws ParserException {
    ...
    for (int i = 0; i < mappingsCount; i++) {
      ...
      if (mappingType != 0) {
        ExoLog.e(TAG, "mapping type other than 0 not supported: " + mappingType);
        continue;
      }
      int submaps;
      if (bitArray.readBit()) {
