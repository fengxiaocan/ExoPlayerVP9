...
public final class WebvttCueParser {
  ...
  private static WebvttCueInfo parseCue(
    ...
    try {
      ...
      ExoLog.w(TAG, "Skipping cue with bad header: " + cueHeaderMatcher.group());
      return null;
    }
    ...
    while (cueSettingMatcher.find()) {
      ...
      try {
        if ("line".equals(name)) {
          ...
          ExoLog.w(TAG, "Unknown cue setting " + name + ":" + value);
        }
      } catch (NumberFormatException e) {
        ExoLog.w(TAG, "Skipping bad cue setting: " + cueSettingMatcher.group());
      }
    }
  }
  ...
  private static @Cue.AnchorType int parseLineAnchor(String s) {
    switch (s) {
      ...
      default:
        ExoLog.w(TAG, "Invalid anchor value: " + s);
  ...
  private static @Cue.VerticalType int parseVerticalAttribute(String s) {
    switch (s) {
      ...
      default:
        ExoLog.w(TAG, "Invalid 'vertical' value: " + s);
        return Cue.TYPE_UNSET;
    }
  }
  ...
  private static @TextAlignment int parseTextAlignment(String s) {
    switch (s) {
      ...
      default:
        ExoLog.w(TAG, "Invalid alignment value: " + s);
  ...
  private static void applyEntity(String entity, SpannableStringBuilder spannedText) {
    switch (entity) {
      ...
      case ENTITY_AMPERSAND:
        ...
        break;
      default:
        ExoLog.w(TAG, "ignoring unsupported entity: '&" + entity + ";'");
        break;
    }
  }
  ...
  private static final class WebvttCueInfoBuilder {
    ...
    private static Layout.Alignment convertTextAlignment(@TextAlignment int textAlignment) {
      switch (textAlignment) {
        ...
        default:
          ExoLog.w(TAG, "Unknown textAlignment: " + textAlignment);
          return null;
      }
    }
