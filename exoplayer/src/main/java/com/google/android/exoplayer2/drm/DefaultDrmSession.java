...
/* package */ class DefaultDrmSession implements DrmSession {
  ...
  public void acquire(@Nullable DrmSessionEventListener.EventDispatcher eventDispatcher) {
    verifyPlaybackThread();
    if (referenceCount < 0) {
      ExoLog.e(TAG, "Session reference count less than zero: " + referenceCount);
      referenceCount = 0;
    }
  ...
  public void release(@Nullable DrmSessionEventListener.EventDispatcher eventDispatcher) {
    verifyPlaybackThread();
    if (referenceCount <= 0) {
      ExoLog.e(TAG, "release() called on a session that's already fully released.");
      return;
    }
    if (--referenceCount == 0) {
  ...
  private void onError(Exception e, @DrmUtil.ErrorSource int errorSource) {
    ...
    ExoLog.e(TAG, "DRM session error", e);
  ...
  private class RequestHandler extends Handler {
    ...
    public void handleMessage(Message msg) {
      ...
      try {
        ...
        ExoLog.w(TAG, "Key/provisioning request produced an unexpected exception. Not retrying.", e);
        response = e;
      }
