  ...
  private final class LoadTask<T extends Loadable> extends Handler implements Runnable {
    ...
    public void run() {
      try {
        ...
        if (!released) {
          ExoLog.e(TAG, "Unexpected exception loading stream", e);
        ...
        if (!released) {
          ExoLog.e(TAG, "OutOfMemory error loading stream", e);
          obtainMessage(MSG_IO_EXCEPTION, new UnexpectedLoaderException(e)).sendToTarget();
        }
      } catch (Error e) {
        ...
        if (!released) {
          ExoLog.e(TAG, "Unexpected error loading stream", e);
      ...
      switch (msg.what) {
        case MSG_FINISH:
          try {
            ...
            // This should never happen, but handle it anyway.
            ExoLog.e(TAG, "Unexpected exception handling load completed", e);
