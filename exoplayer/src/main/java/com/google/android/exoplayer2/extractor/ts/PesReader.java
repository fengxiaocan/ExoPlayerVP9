...
public final class PesReader implements TsPayloadReader {
    ...
    if ((flags & FLAG_PAYLOAD_UNIT_START_INDICATOR) != 0) {
      switch (state) {
        ...
        case STATE_READING_HEADER_EXTENSION:
          ExoLog.w(TAG, "Unexpected start indicator reading extended header");
          break;
        case STATE_READING_BODY:
          ...
          if (payloadSize != C.LENGTH_UNSET) {
            ExoLog.w(TAG, "Unexpected start indicator: expected " + payloadSize + " more bytes");
          }
  ...
  private boolean parseHeader() {
    ...
    if (startCodePrefix != 0x000001) {
      ExoLog.w(TAG, "Unexpected start code prefix: " + startCodePrefix);
    ...
    if (packetLength == 0) {
      ...
      if (payloadSize < 0) {
        ExoLog.w(TAG, "Found negative packet payload size: " + payloadSize);
        payloadSize = C.LENGTH_UNSET;
      }
    }
    return true;
