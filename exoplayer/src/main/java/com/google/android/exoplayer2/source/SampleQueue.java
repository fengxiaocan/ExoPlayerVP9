...
public class SampleQueue implements TrackOutput {
  ...
  /**
   *  Use {@link #createWithDrm(Allocator, DrmSessionManager, EventDispatcher)} instead.
    ...
    if (upstreamAllSamplesAreSyncSamples) {
      ...
      if ((flags & C.BUFFER_FLAG_KEY_FRAME) == 0) {
        ...
        if (!loggedUnexpectedNonSyncSample) {
          ExoLog.w(TAG, "Overriding unexpected non-sync sample for format: " + upstreamFormat);
