...
public final class Cea708Decoder extends CeaDecoder {
    ...
    while (ccData.bytesLeft() >= 3) {
      ...
      if (ccType == DTVCC_PACKET_START) {
        ...
        if (currentDtvCcPacket == null) {
          ExoLog.e(TAG, "Encountered DTVCC_PACKET_DATA before DTVCC_PACKET_START");
          continue;
        }
  ...
  private void processCurrentPacket() {
    ...
    while (captionChannelPacketData.bitsLeft() > 0) {
      ...
      if (serviceNumber == 7) {
        ...
        if (serviceNumber < 7) {
          ExoLog.w(TAG, "Invalid extended service number: " + serviceNumber);
        }
      }
      ...
      if (blockSize == 0) {
        if (serviceNumber != 0) {
          ExoLog.w(TAG, "serviceNumber is non-zero (" + serviceNumber + ") when blockSize is 0");
        }
        break;
      }
      ...
      while (captionChannelPacketData.getPosition() < endBlockPosition) {
        ...
        if (command != COMMAND_EXT1) {
          if (command <= GROUP_C0_END) {
            ...
            ExoLog.w(TAG, "Invalid base command: " + command);
          }
        } else {
          ...
          if (command <= GROUP_C2_END) {
            ...
            ExoLog.w(TAG, "Invalid extended command: " + command);
          }
        }
      }
    }
    ...
    switch (command) {
      ...
      default:
        if (command >= COMMAND_EXT1_START && command <= COMMAND_EXT1_END) {
          ExoLog.w(TAG, "Currently unsupported COMMAND_EXT1 Command: " + command);
          captionChannelPacketData.skipBits(8);
        } else if (command >= COMMAND_P16_START && command <= COMMAND_P16_END) {
          ExoLog.w(TAG, "Currently unsupported COMMAND_P16 Command: " + command);
          captionChannelPacketData.skipBits(16);
        } else {
          ExoLog.w(TAG, "Invalid C0 command: " + command);
        }
    }
  }
  ...
  private void handleC1Command(int command) {
    ...
    switch (command) {
      ...
      case COMMAND_DF7:
        ...
        break;
      default:
        ExoLog.w(TAG, "Invalid C1 command: " + command);
    }
  }
  ...
  private void handleG2Character(int characterCode) {
    switch (characterCode) {
      ...
      case CHARACTER_UPPER_LEFT_BORDER:
        ...
        break;
      default:
        ExoLog.w(TAG, "Invalid G2 character: " + characterCode);
  ...
  private void handleG3Character(int characterCode) {
    if (characterCode == 0xA0) {
      ...
      ExoLog.w(TAG, "Invalid G3 character: " + characterCode);
