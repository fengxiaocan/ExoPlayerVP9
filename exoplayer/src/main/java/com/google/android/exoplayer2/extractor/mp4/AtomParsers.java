...
/* package */ final class AtomParsers {
   ...
   *     atoms (if present).
   */
  public static UdtaInfo parseUdta(LeafAtom udtaAtom) {
  ...
  public static Metadata parseMdtaFromMeta(Atom.ContainerAtom meta) {
    @Nullable LeafAtom hdlrAtom = meta.getLeafAtomOfType(Atom.TYPE_hdlr);
    @Nullable LeafAtom keysAtom = meta.getLeafAtomOfType(Atom.TYPE_keys);
    @Nullable LeafAtom ilstAtom = meta.getLeafAtomOfType(Atom.TYPE_ilst);
    if (hdlrAtom == null
        || keysAtom == null
        || ilstAtom == null
    ...
    while (ilst.bytesLeft() > Atom.HEADER_SIZE) {
      ...
      if (keyIndex >= 0 && keyIndex < keyNames.length) {
        ...
        ExoLog.w(TAG, "Skipped metadata with unknown key index: " + keyIndex);
      }
  ...
  private static TrackSampleTable parseStbl(
    ...
    @Nullable LeafAtom stszAtom = stblAtom.getLeafAtomOfType(Atom.TYPE_stsz);
    if (stszAtom != null) {
      sampleSizeBox = new StszSampleSizeBox(stszAtom, track.format);
    } else {
      @Nullable LeafAtom stz2Atom = stblAtom.getLeafAtomOfType(Atom.TYPE_stz2);
      if (stz2Atom == null) {
        throw ParserException.createForMalformedContainer(
    ...
    @Nullable LeafAtom chunkOffsetsAtom = stblAtom.getLeafAtomOfType(Atom.TYPE_stco);
    ...
    @Nullable LeafAtom stssAtom = stblAtom.getLeafAtomOfType(Atom.TYPE_stss);
    @Nullable ParsableByteArray stss = stssAtom != null ? stssAtom.data : null;
    // Entries are (number of samples, timestamp offset).
    @Nullable LeafAtom cttsAtom = stblAtom.getLeafAtomOfType(Atom.TYPE_ctts);
    ...
    if (rechunkFixedSizeSamples) {
      ...
      for (int i = 0; i < sampleCount; i++) {
        ...
        if (!chunkDataComplete) {
          ExoLog.w(TAG, "Unexpected end of chunk data");
          sampleCount = i;
      ...
      if (childAtomType == Atom.TYPE_avcC) {
        ...
        if (colorSpace == Format.NO_VALUE && colorTransfer == Format.NO_VALUE) {
          ...
          if (colorType == TYPE_nclx || colorType == TYPE_nclc) {
            ...
            ExoLog.w(TAG, "Unsupported color type: " + Atom.getAtomTypeString(colorType));
          }
        }
      }
  ...
  private static Pair<long[], long[]> parseEdts(Atom.ContainerAtom edtsAtom) {
    @Nullable LeafAtom elstAtom = edtsAtom.getLeafAtomOfType(Atom.TYPE_elst);
    if (elstAtom == null) {
      return null;
    }
  ...
  /* package */ static final class StszSampleSizeBox implements SampleSizeBox {
    ...
    public StszSampleSizeBox(LeafAtom stszAtom, Format trackFormat) {
      data = stszAtom.data;
