  ...
  private static final class InternalHandler extends Handler {
    ...
    private void initialize(int notMetRequirements) {
      ...
      try {
        ...
        ExoLog.e(TAG, "Failed to load index.", e);
        downloads.clear();
      } finally {
    ...
    private void setStopReason(@Nullable String id, int stopReason) {
      if (id == null) {
        ...
        try {
          ...
          ExoLog.e(TAG, "Failed to set manual stop reason", e);
        }
      } else {
        ...
        if (download != null) {
          ...
          try {
            ...
            ExoLog.e(TAG, "Failed to set manual stop reason: " + id, e);
          }
        }
      }
      syncTasks();
    ...
    private void removeDownload(String id) {
      ...
      if (download == null) {
        ExoLog.e(TAG, "Failed to remove nonexistent download: " + id);
        return;
      }
    ...
    private void removeAllDownloads() {
      ...
      try (DownloadCursor cursor = downloadIndex.getDownloads(STATE_COMPLETED, STATE_FAILED)) {
        ...
        ExoLog.e(TAG, "Failed to load downloads.");
      }
      for (int i = 0; i < downloads.size(); i++) {
      ...
      try {
        ...
        ExoLog.e(TAG, "Failed to update index.", e);
      }
    ...
    private void onTaskStopped(Task task) {
      ...
      if (finalException != null) {
        ExoLog.e(TAG, "Task failed: " + task.request + ", " + isRemove, finalException);
      }
    ...
    private void onRemoveTaskStopped(Download download) {
      if (download.state == STATE_RESTARTING) {
        ...
        try {
          ...
          ExoLog.e(TAG, "Failed to remove from database");
        }
        DownloadUpdate update =
            new DownloadUpdate(
                download,
    ...
    private Download getDownload(String id, boolean loadFromIndex) {
      ...
      if (loadFromIndex) {
        try {
          ...
          ExoLog.e(TAG, "Failed to load download: " + id, e);
        }
      }
      return null;
    }
