    ...
    switch (intentAction) {
      ...
      case ACTION_ADD_DOWNLOAD:
        ...
        if (downloadRequest == null) {
          ExoLog.e(TAG, "Ignored ADD_DOWNLOAD: Missing " + KEY_DOWNLOAD_REQUEST + " extra");
        } else {
      ...
      case ACTION_REMOVE_DOWNLOAD:
        if (contentId == null) {
          ExoLog.e(TAG, "Ignored REMOVE_DOWNLOAD: Missing " + KEY_CONTENT_ID + " extra");
        } else {
      ...
      case ACTION_SET_STOP_REASON:
        if (!Assertions.checkNotNull(intent).hasExtra(KEY_STOP_REASON)) {
          ExoLog.e(TAG, "Ignored SET_STOP_REASON: Missing " + KEY_STOP_REASON + " extra");
        } else {
      ...
      case ACTION_SET_REQUIREMENTS:
        ...
        if (requirements == null) {
          ExoLog.e(TAG, "Ignored SET_REQUIREMENTS: Missing " + KEY_REQUIREMENTS + " extra");
        } else {
          downloadManager.setRequirements(requirements);
        }
        break;
      default:
        ExoLog.e(TAG, "Ignored unrecognized action: " + intentAction);
        break;
    }
  ...
  private static final class DownloadManagerHelper implements DownloadManager.Listener {
    ...
    public boolean updateScheduler() {
      ...
      if (scheduler.schedule(requirements, servicePackage, ACTION_RESTART)) {
        ...
        ExoLog.w(TAG, "Failed to schedule restart");
        cancelScheduler();
        return false;
      }
    }
    ...
    public void onDownloadChanged(
      ...
      if (serviceMayNeedRestart() && needsStartedService(download.state)) {
        ...
        ExoLog.w(TAG, "DownloadService wasn't running. Restarting.");
        restartService();
      }
    }
    ...
    private void restartService() {
      if (foregroundAllowed) {
        try {
          ...
          ExoLog.w(TAG, "Failed to restart (foreground launch restriction)");
        }
      } else {
        ...
        try {
          ...
          ExoLog.w(TAG, "Failed to restart (process is idle)");
        }
      }
    }
  }
