    ...
    if (codecDrainAction == DRAIN_ACTION_FLUSH_AND_UPDATE_DRM_SESSION) {
      ...
      if (Util.SDK_INT >= 23) {
        try {
          ...
          ExoLog.w(TAG, "Failed to update the DRM session, releasing the codec instead.", e);
          releaseCodec();
          return true;
        }
      }
      ...
      try {
        try {
          ...
          if (codecInfo == preferredCodecInfo) {
            ...
            ExoLog.w(TAG, "Preferred decoder instantiation failed. Sleeping for 50ms then retrying.");
        ...
        ExoLog.w(TAG, "Failed to initialize decoder: " + codecInfo, e);
